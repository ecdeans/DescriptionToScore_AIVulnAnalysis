...
def generate_vector_and_scores(cve_descriptions):
    ...
    From each CVE description, identify the following Base Metrics:  
    ...
    Example 6:
    CVE Description: An arbitrary file upload vulnerability in the File preview function of Raingad IM v4.1.4 allows attackers to execute arbitrary code via uploading a crafted PDF file.
    Attack Complexity: LOW
    Attack Vector: LOCAL
    Privileges Required: NONE
    User Interaction: REQUIRED
    ...
    Output: LOW | LOCAL | HIGH | NONE | UNCHANGED | HIGH | LOW | LOW
    
    Example 8:
    CVE Description: apphp js-object-resolver < 3.1.1 is vulnerable to Prototype Pollution via Module.setNestedProperty.
    Attack Complexity: LOW
    Attack Vector: NETWORK
    Privileges Required: NONE
    User Interaction: REQUIRED
    ...
    Integrity Impact: HIGH
    Availability Impact: HIGH
    Output: LOW | NETWORK | NONE | REQUIRED | UNCHANGED | LOW | HIGH | HIGH
    
    Example 9:
    CVE Description: Redmi router RB03 v1.0.57 is vulnerable to TCP DoS or hijacking attacks. An attacker in the same WLAN as the victim can disconnect or hijack the traffic between the victim and any remote server by sending out forged TCP RST messages to evict NAT mappings in the router.
    Attack Complexity: LOW
    Attack Vector: ADJACENT_NETWORK
    Privileges Required: LOW
    User Interaction: REQUIRED
    ...
    Integrity Impact: LOW
    Availability Impact: LOW
    Output: LOW | PHYSICAL | NONE | NONE | UNCHANGED | LOW | LOW | LOW
...
def main():
    folder_num = 35
    while folder_num <= 56:  # Adjust folder range if needed
